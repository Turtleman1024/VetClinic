...
public class PatientController : Controller
{
    ...
    {
        var patients = await _vetClinic.GetAllPatientsAsync(HttpContext.RequestAborted);
    ...
    {
        var patient = await _vetClinic.GetPatientByIdAsync(patientId, HttpContext.RequestAborted);
        if (patient == null)
        {
    ...
    {
        var activePatients = await _vetClinic.GetActivePatientsAsync(HttpContext.RequestAborted);
    ...
    {
        var patients = await _vetClinic.GetPatientsNameAsync(name, HttpContext.RequestAborted);
        if (patients == null)
        {
    ...
    {
        var patient = await _vetClinic.CreatePatientAsync(newPatient, HttpContext.RequestAborted);
        if (patient == null)
        {
    ...
    public async Task<IActionResult> UpdatePatientAsync(int patientId, [FromBody] JsonPatchDocument<Patient> patientPatch)
    {
        ...
        {
            var patient = await _vetClinic.UpdatePatientAsync(patientId, patientPatch, HttpContext.RequestAborted);
            if (patient != null)
            {
                return Ok(patient);
            }
    ...
    {
        var deleted = await _vetClinic.DeletePatientByIdAsync(patientId, HttpContext.RequestAborted);
