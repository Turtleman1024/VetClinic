...
public class OwnerController : ControllerBase
{
    ...
    {
        var owners = await _vetClinic.GetAllOwnersAsync(HttpContext.RequestAborted);
        if (((owners?.Count ?? 0) == 0))
        {
    ...
    {
        var owner = await _vetClinic.GetOwnerByIdAsync(ownerId, HttpContext.RequestAborted);
        if (owner == null)
        {
    ...
    {
        var owners = await _vetClinic.SearchForOwnerAsync(searchValue, HttpContext.RequestAborted);
        if (owners == null)
        {
    ...
    {
        var owner = await _vetClinic.CreateOwnerAsync(newOwner, HttpContext.RequestAborted);
    ...
    public async Task<IActionResult> UpdateOwnerAsync(int ownerId, [FromBody] JsonPatchDocument<Owner> ownerPatch)
    {
        ...
        {
            var owner = await _vetClinic.UpdateOwnerAsync(ownerId, ownerPatch, HttpContext.RequestAborted);
            if (owner != null)
            {
                return Ok(owner);
            }
    ...
    {
        var deleted = await _vetClinic.DeleteOwnerByIdAsync(ownerId, HttpContext.RequestAborted);
